...
#include <openssl/x509.h>   // <-- needed for i2d_PUBKEY / d2i_PUBKEY
...
static Alg defaultAlgFromEnv() {
    ...
    if (!env) return Alg::DILITHIUM_2; // current default in your apps
    std::string s(env);
...
static Alg algFromPrefixed(const std::string& s) {
    ...
    // no prefix => assume current default (for backward compat you can force PQC)
    return Alg::DILITHIUM_2;
}
